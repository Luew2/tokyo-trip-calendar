...
const JapanTripCalendar = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Here we're using the JSON directly
        const data = {
        ...
        if (data && data.schedule && data.schedule.length > 0) {
          initialExpandedState[data.schedule[0].date] = true;
        }
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <p className="text-lg text-gray-600">Loading your trip itinerary...</p>
      </div>
    );
  }
  ...
  if (!tripData) {
    return (
      <div className="flex justify-center items-center h-64">
        <p className="text-lg text-red-600">Failed to load trip data</p>
      </div>
    );
  }
